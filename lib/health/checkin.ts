...
export type HealthCheckinPortWriteResult =
  | { ok: true }
  | { ok: false; code: HealthCheckinPortWriteCode };

export interface HealthCheckinSheetPort {
  read(): Promise<ReadTabResult>;
  writeRow(rowNumber: number, row: readonly HealthCheckinCell[]): Promise<HealthCheckinPortWriteResult>;
}
...
export function inspectHealthCheckinTable(values: readonly (readonly HealthCheckinCell[])[]): HealthCheckinInspection {
...
export function buildHealthCheckinSnapshot(
  ...
  if (!read.ok) {
    return {
      state:
        read.code === 'missing-tab' || read.code === 'not-configured' || read.code === 'permission-error'
          ? 'unavailable'
          : 'error',
      saved: false,
      canWrite: false,
