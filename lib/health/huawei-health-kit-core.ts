...
export const HUAWEI_OAUTH_AUTH_URL = 'https://oauth-login.cloud.huawei.com/oauth2/v3/authorize';
...
export function resolveHuaweiHealthSetupConfig(
  env: EnvLike,
): { ok: true; config: HuaweiHealthSetupConfig } | { ok: false; reason: 'not-configured' } {
  ...
  if (!clientId || !clientSecret || !redirectUri) return { ok: false, reason: 'not-configured' };
...
export function resolveHuaweiHealthRuntimeConfig(
  ...
  if (!setup.ok || !refreshToken) return { ok: false, reason: 'not-configured' };
...
export function isAllowedHuaweiHealthRedirect(value: string): boolean {
  try {
    ...
    return url.protocol === 'https:' && allowedHosts.has(url.hostname) && url.pathname.startsWith('/healthkit/');
  } catch {
    return false;
  }
}
...
export function extractHuaweiHrvSamples(payload: unknown): HuaweiHrvSample[] {
  ...
  const visit = (node: unknown): void => {
    ...
    if (record.dataTypeName === HUAWEI_HRV_DATA_TYPE && Array.isArray(record.value)) {
          ...
          typeof field.integerValue === 'number'
            ...
            : typeof field.floatValue === 'number'
              ...
              : typeof field.longValue === 'number'
                ? field.longValue
                : null;
        if (candidate !== null && Number.isFinite(candidate) && candidate > 0 && candidate <= 200) {
          samples.push({
            rmssdMs: candidate,
...
export function summarizeHuaweiHrvSamples(samples: readonly HuaweiHrvSample[]) {
  if (samples.length === 0) {
    return { available: false as const, sampleCount: 0, minRmssdMs: null, maxRmssdMs: null };
  }
