...
export function buildWeeklyCardioSummary(input: {
  ...
  const contributions: GymCardioContribution[] = input.cardioSessions
    .filter((session) => session.date >= weekStart && session.date <= input.today)
    .map(contributionFromSession)
    .filter((item): item is GymCardioContribution => item !== null);

  const footballDates = new Set(
    contributions.filter((item) => item.kind === 'football').map((item) => item.date),
  );
  let uncreditedWalkingDays = 0;
  ...
  if (input.healthSourceAvailable && input.healthPeriodStart && distance && speed) {
    const count = Math.max(distance.series.length, speed.series.length, steps?.series.length ?? 0);
    for (let index = 0; index < count; index += 1) {
      const date = addDays(input.healthPeriodStart, index);
      if (date < weekStart || date > input.today) continue;

      const daySteps = steps?.series[index] ?? null;
      const km = distance.series[index] ?? null;
      const kmh = speed.series[index] ?? null;

      if (footballDates.has(date)) {
        if ((daySteps ?? 0) > 0) uncreditedWalkingDays += 1;
        continue;
      }

      if (km === null || kmh === null || km <= 0 || kmh <= 0) {
        if ((daySteps ?? 0) > 0) uncreditedWalkingDays += 1;
        continue;
      }

      const met = walkingMet(kmh);
      if (met === null) {
        if ((daySteps ?? 0) > 0) uncreditedWalkingDays += 1;
        continue;
      }
      ...
      contributions.push({
        ...
        kind: 'walking',
        label: 'Caminata',
        durationMinutes: rounded(minutes, 1),
        met,
        metMinutes: rounded(minutes * met, 0),
        confidence: 'low',
        detail: `${rounded(kmh, 1)} km/h · ${rounded(km, 1)} km; estimación con datos de movimiento`,
      });
    }
  } else if (input.healthSourceAvailable && steps) {
