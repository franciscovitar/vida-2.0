...
test('la afinidad retrospectiva completa una serie sólo cuando falta una predicción privada', () => {
  ...
  assert.equal(withRetrospectiveSeriesAffinity([title('series', null)], fallback)[0]!.estimatedAffinity, 7.625);
  assert.equal(withRetrospectiveSeriesAffinity([title('series', 8.9)], fallback)[0]!.estimatedAffinity, 8.9);
});
