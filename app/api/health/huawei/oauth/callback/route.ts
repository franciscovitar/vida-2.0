...
export async function GET(request: NextRequest) {
  ...
  const cookieState =
    request.cookies.get(HUAWEI_OAUTH_STATE_COOKIE)?.value ?? null;
  if (
    !code ||
    !state ||
    !cookieState ||
    !safeEqualStrings(state, cookieState)
  ) {
  ...
  if (!exchanged.ok) {
    return html(
      `<h1>Token exchange failed</h1><p>${escapeHtml(exchanged.reason)}</p>`,
      400,
    );
  }
  ...
  return html(
    `<!doctype html><html lang="es"><meta charset="utf-8"><meta name="referrer" content="no-referrer"><title>Huawei Health token</title><body style="font-family:system-ui;max-width:760px;margin:40px auto;padding:0 20px"><h1>Autorización lista</h1><p>Copiá este valor a <code>HUAWEI_HEALTH_REFRESH_TOKEN</code> en <code>.env.local</code>. No lo pegues en chats, capturas ni commits.</p><pre style="white-space:pre-wrap;word-break:break-all;border:1px solid #ccc;padding:12px">${token}</pre><p>Después reiniciá <code>npm run dev</code> y abrí <code>/api/health/huawei/hrv-probe?days=7</code>.</p></body></html>`,
  );
}
