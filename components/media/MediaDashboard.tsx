  ...
  return (
    <div className={styles.stack}>
      ...
      <section className={styles.hero} aria-label="Resumen de Media">
        <div className={styles.mediumTabs} role="tablist" aria-label="Tipo de contenido">
          <button
            type="button"
            role="tab"
            aria-selected={filters.medium === 'movie'}
            className={styles.mediumTab}
        ...
        <p className={styles.bankPrinciple}>
      ...
      <section className={styles.controls} aria-label="Filtros de Media">
        <div className={styles.collectionTabs} role="group" aria-label="Colección">
        ...
        <div className={styles.filterGrid}>
          ...
          <label>
            <span>Década</span>
            <select value={filters.decade} onChange={(event) => patchFilters({ decade: event.target.value })}>
      ...
      <div className={styles.resultHeader}>
        ...
        {!options.hasScores ? (
          <span className={styles.scoreNote}>Scores personales aún no certificados/persistidos.</span>
        ) : null}
      </div>
      ...
      {visible.length === 0 ? (
        ...
        <section className={styles.grid} aria-label="Resultados de Media">
          {visible.map((item) => {
            ...
            return (
              <article key={item.key} className={styles.titleCard}>
                ...
                <div className={styles.scores}>
                  {item.rating !== null ? (
                    <div className={styles.observedScore}>
                      <span>Mi nota</span>
                      <strong>{score(item.rating)}</strong>
                    </div>
                  ) : null}
                  {hasInference ? (
                    <div className={styles.inferredScores}>
